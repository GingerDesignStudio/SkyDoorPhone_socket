...
io.on("connection", function (socket) {
    ...
    if (name == "entrance") {
      ...
      if (devices[1] == "") {
        ...
        io.to(id).emit(
          "server_to_client",
          "Error",
          "Recievers are already full.",
        );
        console.log(
          "Error:デバイスが既に6台使われています。これ以上入室できません。",
        );
        return;
      }
      socket.join(room);
      ...
      io.to(id).emit("server_to_client", "TESTOK", "server");
      console.log("OK:テストに成功しました。");
    } else {
  ...
  socket.on("disconnect", function () {
    ...
    // メンバーリストを送信
    io.to(room).emit("server_to_client_showmember", getMemberList(), "server");
  });
});
